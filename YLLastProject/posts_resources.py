...
from data import db_session, Posts, User
...
class NotesResource(Resource):
    def get(self, notes_id):
        ...
        note = session.query(Posts).get(notes_id)
    ...
    def delete(self, notes_id):
        ...
        notes = session.query(Posts).get(notes_id)
...
class NotesListResource(Resource):
    def get(self):
        ...
        return jsonify({'notes': [item.to_dict(
            only=('id', 'content', 'user.name', 'created_date')) for item in posts]})
